...
**Syntax**:
```bash
/adrs-management:adr-map [--project-dir=PATH] [--context-dir=PATH] [--output-dir=PATH]
```
...
/adrs-management:adr-map --project-dir=/path/to/project

# Map with architecture documentation context
/adrs-management:adr-map --context-dir=docs/architecture

# Full control with custom paths
/adrs-management:adr-map --project-dir=/legacy --context-dir=/legacy/docs --output-dir=analysis/adrs
```
...
**Prerequisites**: Run `/adrs-management:adr-map` first if `mapping.md` doesn't exist.

**Syntax**:
```bash
/adrs-management:adr-identify [module-ids] [--output-dir=PATH] [--adrs-dir=PATH]
```
...
/adrs-management:adr-identify AUTH

# Analyze multiple modules in parallel
/adrs-management:adr-identify AUTH DATA API

# Custom output directory
/adrs-management:adr-identify --output-dir=output/adrs AUTH

# Use existing ADRs for context
/adrs-management:adr-identify --adrs-dir=docs/adrs/generated AUTH
```
...
**Prerequisites**: Run `/adrs-management:adr-identify` first to create potential ADRs.

**Syntax**:
```bash
/adrs-management:adr-generate [modules] [--context-dir=PATH] [--language=LOCALE] [--include-consider] [--output-dir=PATH]
```
...
/adrs-management:adr-generate --all

# Generate for specific module
/adrs-management:adr-generate BILLING

# Generate multiple modules in parallel
/adrs-management:adr-generate BILLING API DATA

# Include medium priority ADRs
/adrs-management:adr-generate --include-consider BILLING

# Generate in Portuguese
/adrs-management:adr-generate --language=pt-BR --include-consider BILLING API

# With strategic context
/adrs-management:adr-generate --context-dir=docs/context/ BILLING

# Custom output directory
/adrs-management:adr-generate --output-dir=output/adrs BILLING
```
...
**Prerequisites**: Run `/adrs-management:adr-generate` first to create formal ADRs.

**Syntax**:
```bash
/adrs-management:adr-link [modules] [--validate] [--report-only] [--adrs-path=PATH] [--output-dir=PATH]
```
...
/adrs-management:adr-link BILLING API

# Validate existing links
/adrs-management:adr-link --validate

# Generate report only
/adrs-management:adr-link --report-only

# Custom ADRs path
/adrs-management:adr-link --adrs-path=output/adrs/generated

# Generate report in specific directory
/adrs-management:adr-link --report-only --output-dir=docs/reports/
```
...
### Phase 1: Codebase Mapping (`/adrs-management:adr-map`)
...
### Phase 2: ADR Identification (`/adrs-management:adr-identify`)
...
### Phase 3: ADR Generation (`/adrs-management:adr-generate`)
...
### Post-Processing: Relationship Linking (`/adrs-management:adr-link`)
...
/adrs-management:adr-identify AUTH DATA API BILLING

# Step 3: Generate formal ADRs
/adrs-management:adr-generate --include-consider AUTH DATA API BILLING
...
/adrs-management:adr-map
/adrs-management:adr-identify AUTH
/adrs-management:adr-generate AUTH

# Add more modules later
/adrs-management:adr-identify DATA API
/adrs-management:adr-generate DATA API
...
```bash
/adrs-management:adr-map --output-dir=custom/adrs
/adrs-management:adr-identify --output-dir=custom/adrs AUTH
/adrs-management:adr-generate --output-dir=custom/adrs AUTH
/adrs-management:adr-link --adrs-path=custom/adrs/generated
```
...
| Issue | Solution |
|-------|----------|
| `mapping.md` not found | Run `/adrs-management:adr-map` first |
...
| Links not created | Run `/adrs-management:adr-link` after generating ADRs |
| Module not found | Check spelling or run `/adrs-management:adr-map` to see available modules |
